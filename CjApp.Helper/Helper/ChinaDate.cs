...
namespace CjApp.Helper.Helper
{
    ...
    {
        private static readonly ChineseLunisolarCalendar China = new();
        private static readonly Hashtable GHoliday = new();
        private static readonly Hashtable NHoliday = new();
        ...
        public static string GetChinaDate(DateTime dt)
        {
            if (dt > China.MaxSupportedDateTime || dt < China.MinSupportedDateTime)
            {
                ...
                throw new Exception(
                    $"日期超出范围！必须在{China.MinSupportedDateTime:yyyy-MM-dd}到{China.MaxSupportedDateTime:yyyy-MM-dd}之间！");
            }
        ...
        public static string GetYear(DateTime dt)
        {
            ...
            var yearDZ = " 子丑寅卯辰巳午未申酉戌亥";
            var yearSX = " 鼠牛虎兔龙蛇马羊猴鸡狗猪";
